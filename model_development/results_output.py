...
#Defining variables
    ...
    meal_price =[round(m.objVal, 2)]

    # Adding items and their servings to the lists
    for var in m.getVars():
        if var.x > 0 and var.Varname in menu_item:
            item_section = menu_data.loc[menu_data['item_name'] == var.Varname, 'menu_section'].iloc[0]
            section_var.append(item_section)
    ...
    df = pd.DataFrame({
        ...
        "Menu": menu_items + [""]*((len(nutrients))),
        "Servings": servings + [""]*(len(nutrients)),
        "Meal Price": meal_price + [""]*(len(nutrients)+len(menu_items)-1),
